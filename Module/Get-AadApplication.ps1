...
function Get-AadApplication
{
    ...
    if($Id)
    {
        ...
        if ($isGuid -and -not $app) {
            ...
            $app = $null
            $app = try { Get-AzureADObjectByObjectId -ObjectId $Id } catch {}
...
function GetAadAppByAppId
{
    ...
    $app = Get-AzureAdApplication -filter "AppId eq '$Id'"
    if ($app) { 
...
function GetAadAppByDisplayName
{
    ...
    $app = Get-AzureADApplication -filter "DisplayName eq '$Id'"
...
function LookupAllApplications()
{
    if(-not $Global:Applications)
    {
        ...
        $Global:Applications = Get-AzureADApplication -All $true
    }
